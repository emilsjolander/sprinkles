...
public final class ManyQuery<T extends Model> {
	...
	public void getAsyncWithUpdates(LoaderManager lm,
			OnQueryResultHandler<List<T>> handler, Class<? extends Model>... respondsToUpdatedOf) {
		
		// list of classes which when updated will trigger a reload of the dataset
		final List<Class<? extends Model>> listOfDependencies = Arrays.asList(respondsToUpdatedOf);
		listOfDependencies.add(resultClass);
		
		final int loaderId = sqlQuery.hashCode();
		lm.initLoader(loaderId, null,
				getLoaderCallbacks(sqlQuery, resultClass, handler, true, listOfDependencies));
	}

	private LoaderCallbacks<Cursor> getLoaderCallbacks(final String sqlQuery,
			final Class<T> resultClass,
			final OnQueryResultHandler<List<T>> handler,
			final boolean getUpdates, final List<Class<? extends Model>> respondsToUpdatedOf) {
	...
	public void getAsync(android.support.v4.app.LoaderManager lm, OnQueryResultHandler<List<T>> handler) {
		final int loaderId = sqlQuery.hashCode();
		lm.initLoader(loaderId, null,
				getSupportLoaderCallbacks(sqlQuery, resultClass, handler, false, null));
	}
	...
	public void getAsyncWithUpdates(android.support.v4.app.LoaderManager lm,
		...
		final int loaderId = sqlQuery.hashCode();
		lm.initLoader(loaderId, null,
				getSupportLoaderCallbacks(sqlQuery, resultClass, handler, true, listOfDependencies));
	}

	private android.support.v4.app.LoaderManager.LoaderCallbacks<Cursor> getSupportLoaderCallbacks(final String sqlQuery,
			final Class<T> resultClass, final OnQueryResultHandler<List<T>> handler,
			final boolean getUpdates, final List<Class<? extends Model>> respondsToUpdatedOf) {
		return new android.support.v4.app.LoaderManager.LoaderCallbacks<Cursor>() {
			...
			public android.support.v4.content.Loader<Cursor> onCreateLoader(int id, Bundle args) {
				return new SupportCursorLoader(Sprinkles.sInstance.mContext, sqlQuery,
						respondsToUpdatedOf);
			}
		};
	}
